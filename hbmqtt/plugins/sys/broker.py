...
class BrokerSysPlugin:
    ...
    def broadcast_dollar_sys_topics(self):
        ...
        tasks = deque()
        tasks.append(
            self.schedule_broadcast_sys_topic("load/bytes/received", int_to_bytes_str(self._stats[STAT_BYTES_RECEIVED]))
        )
        ...
        tasks.append(
            self.schedule_broadcast_sys_topic("clients/total", int_to_bytes_str(client_connected + client_disconnected))
        )
        ...
        tasks.append(
            self.schedule_broadcast_sys_topic("messages/publish/sent", int_to_bytes_str(self._stats[STAT_PUBLISH_SENT]))
        )
        tasks.append(
            self.schedule_broadcast_sys_topic("messages/retained/count", int_to_bytes_str(len(self.context.retained_messages)))
        )
